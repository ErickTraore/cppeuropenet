  ...
  it('récupère les messages presse générale puis supprime uniquement les 4 enregistrements E2E', () => {
    const runVerification = () => {
      cy.get('@accessToken').then((token) => {
        cy.request({
          ...
          if (res.status === 403) {
            cy.log('Vérification cleanup ignorée: token user-backend non accepté par presseGenerale-backend');
            return;
          }
          expect(res.status).to.eq(200);
    ...
    cy.get('@accessToken').then((token) => {
      cy.request({
        ...
        if (res.status === 403) {
          cy.log('Cleanup ignoré: token user-backend non accepté par presseGenerale-backend');
          return;
        }
        expect(res.status).to.eq(200);
        const messages = Array.isArray(res.body) ? res.body : [];
